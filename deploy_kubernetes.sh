...
if ansible-playbook -i "$ANSIBLE_INVENTORY" "$KUBERNETES_PLAYBOOK" -v; then
    ...
    info "- Jellyfin media server deployed with high availability (2 replicas)"
    info "- All services available via NodePort"
    echo ""
    info "Service Access URLs:"
    info "1. Grafana: http://192.168.4.63:30300 (admin/admin)"
    info "2. Prometheus: http://192.168.4.63:30090"
    info "3. Loki: http://192.168.4.63:31100"
    info "4. Jellyfin: http://192.168.4.61:30096 (4K streaming ready)"
    echo ""
    info "Next steps:"
    info "1. Configure firewall rules if needed"
    info "2. Set up DNS for jellyfin.vmstation.local (ingress)"
    info "3. Configure Jellyfin media libraries (/media mount)"
    info "4. Set up ingress for external access (optional)"
