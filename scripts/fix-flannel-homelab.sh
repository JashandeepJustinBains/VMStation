...
ssh 192.168.4.62 'sudo sed -i "s/^SELINUX=enforcing/SELINUX=permissive/" /etc/selinux/config 2>/dev/null || echo "SELinux config not modified"'
echo ""

echo "2. Ensuring all kernel modules are loaded..."
ssh 192.168.4.62 'sudo modprobe br_netfilter overlay nf_conntrack vxlan'
echo ""

echo "3. Verifying iptables is in legacy mode..."
ssh 192.168.4.62 'sudo alternatives --set iptables /usr/sbin/iptables-legacy 2>/dev/null || echo "iptables-legacy already set or not available"'
...
ssh 192.168.4.62 'ls -la /opt/cni/bin/ 2>/dev/null | grep -E "flannel|bridge|host-local" || echo "CNI binaries missing - flannel should install them"'
echo ""

echo "5. Removing any stale Flannel interfaces..."
ssh 192.168.4.62 'sudo ip link delete flannel.1 2>/dev/null || echo "No flannel.1 interface to delete"'
ssh 192.168.4.62 'sudo ip link delete cni0 2>/dev/null || echo "No cni0 interface to delete"'
echo ""
